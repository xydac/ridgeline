...
- ~~Business Memory identifies streams and metrics by `<connector type>.<stream>.<column>`, ignoring the connector's configured `name` and its parent product.~~ Fixed in v0.3.0: FQ names now use `<product>.<connector-name>.<stream>.<column>`. Schema migration v14 rewrites existing rows; `memory metrics --legacy-fqn` shows the deprecated form for one release.
...
- `monitor add` accepts any string as `--metric` and never checks the catalog, so a typo silently registers a watch that can never fire. Validate the metric name against `bm_metrics` at registration time and reject unknowns with the closest matches suggested.
- `monitor run` returns exit code 0 whether or not rules triggered, so a script wrapping it cannot distinguish "quiet" from "fired" without parsing `--json` output. Return a distinct exit code (or add a `--exit-nonzero-on-trigger` flag) so shell pipelines can page on violations.
