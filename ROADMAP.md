...
- The MCP server exposes only five of the seven CLI reasoning primitives; `forecast`, `recommend`, and `monitor` are reachable from the shell but not from an agent. Expose them as MCP tools so agents can reason over projections, focus lists, and threshold rules without shelling out.
